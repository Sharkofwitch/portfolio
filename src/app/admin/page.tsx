...
export default function AdminPage() {
  ...
  const { register, handleSubmit, reset, watch, setValue, formState: { errors } } = useForm<PhotoFormData>();
    ...
    const handleAuth = async () => {
      ...
      if (status === 'unauthenticated') {
        ...
        if (!session?.user?.role) {
          console.log('⚠️ User has no role defined in session. Current session:', session);
          router.push('/');
        } else if (session.user.role !== 'admin') {
          console.log(`⚠️ User does not have admin role (has ${session.user.role}), redirecting to home`);
          router.push('/');
        } else {
    ...
    return (
      <div className="container mx-auto px-4 py-8 flex flex-col items-center justify-center min-h-[60vh]">
        ...
        <p className="text-gray-400 text-sm mt-2">
          {status === 'loading' ? 'Authenticating user...' : 'Loading content...'}
        </p>
      </div>
    );
  }
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            {previewUrl && (
              <div className="relative aspect-video">
                <Image
                  src={previewUrl?.startsWith('/photos/') 
